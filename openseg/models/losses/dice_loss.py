...
def dice_score(predicts: torch.Tensor, labels: torch.Tensor, smooth: float = 1.0):
    ...
    dice_score = (numer + smooth) / (denor + smooth)
...
class DiceLoss(torch.nn.Module):
    ...
    def forward(self, predicts: torch.Tensor, labels: torch.Tensor):
        ...
        numer = torch.sum(torch.mul(predicts, labels), dim=(0, 1, 2)) * 2.0 + self.smooth
